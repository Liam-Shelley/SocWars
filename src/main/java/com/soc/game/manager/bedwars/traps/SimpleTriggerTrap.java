...
public class SimpleTriggerTrap extends AbstractTrap {
    public interface TriggerFunction {
        void trigger(ServerPlayerEntity player, float trapAmplifier);
    }
    ...
    public static final AbstractTrap LAUNCH = register(new SimpleTriggerTrap("launch", Items.PISTON.getDefaultStack(), 5 * 20, (enemy, trapAmplifier) -> enemy.networkHandler.sendPacket(new EntityVelocityUpdateS2CPacket(enemy.getId(), new Vec3d(0d, trapAmplifier * 0.4d, 0d)))));
    public static final AbstractTrap STEVIE_WONDER = register(new SimpleTriggerTrap("steve_harvey", Items.SCULK_VEIN.getDefaultStack(), 8 * 20, (enemy, trapAmplifier) -> {
        enemy.addStatusEffect(new StatusEffectInstance(StatusEffects.BLINDNESS, (int)(trapAmplifier * 8 * 20), 1, false, true, true));
        enemy.addStatusEffect(new StatusEffectInstance(StatusEffects.SLOWNESS, (int)(trapAmplifier * 8 * 20), 1, false, true, true));
    }));
    ...
    public static final AbstractTrap ENLARGEMENT = register(new SimpleTriggerTrap("enlargement", EatFunctionFood.BIGGENING_PILLS.getDefaultStack(), 10 * 20, (enemy, trapAmplifier) -> {
        scaleEntity(enemy, 2f);
        Events.getInstance().scheduleEvent(() -> resetScale(enemy), (int)(trapAmplifier * 10 * 20));
    }));
    ...
    private final TriggerFunction enemyTriggerFunction;

    public SimpleTriggerTrap(String id, ItemStack icon, int time, Consumer<ServerPlayerEntity> enemyTriggerFunction) {
        super(id, icon, time);
        this.enemyTriggerFunction = (enemy, trapAmplifier) -> enemyTriggerFunction.accept(enemy);
    }

    public SimpleTriggerTrap(String id, ItemStack icon, int time, TriggerFunction enemyTriggerFunction) {
        super(id, icon, time);
    ...
    public SimpleTriggerTrap(String id, ItemStack icon, int duration, RegistryEntry<StatusEffect> effect, int amplifier) {
        ...
        this.enemyTriggerFunction = (enemy, trapAmplifier) -> enemy.addStatusEffect(new StatusEffectInstance(effect, (int)(duration * trapAmplifier), amplifier, false, false));
    }
    ...
    public void trigger(Vec3d pos, AbstractGameManager<?, ?, ?> manager, Collection<ServerPlayerEntity> enemies, DyeColor team, float trapAmplifier) {
        enemies.forEach(enemy -> this.enemyTriggerFunction.trigger(enemy, trapAmplifier));
    }
}
