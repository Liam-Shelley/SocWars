...
public class RedirectorAbility extends AbstractAbility {
    public interface TriggerFunction {
        ResultModifier trigger(Vec3d pos, AbstractGameManager<?, ?, ?> manager, Collection<ServerPlayerEntity> enemiesInRange, DyeColor owningTeam, AbstractTrap trapTriggerFunction);
    }
    ...
    public static final AbstractAbility TRAP_AMPLIFIER = register(new RedirectorAbility("trap_amplifier", Items.GLASS_PANE.getDefaultStack(), 30 * 20, TriggerReason.TRAP_MODIFIER, (pos, manager, enemiesInRange, owningTeam, trapTriggerFunction) -> {
        return new ResultModifier(true, 2f);
    }));
